...
## Note on Adding Suffix in 'gt()' {.smaller}

- The `suffixing` argument in `fmt_number()` allows you to append units after numbers.

- However, be cautious when using suffixes. 

::: {.callout-caution .fragment}
## large-number suffixing
When you use `fmt_integer()` in `gt`, you can ask it to automatically scale large numbers (like thousands or millions) and append the right suffix such as:

  - K → thousands
  - M → millions
  - B → billions

In our case, If we use the `suffixing = " g"` argument for adding "g" suffix to `avg_mass`, it will **autoscale large numbers** and then add the " g" suffix, which is not what we want for body mass in grams.
:::
